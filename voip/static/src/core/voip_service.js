// voip/static/src/core/voip_service.js
...
export class Voip {
    ...
    constructor(env, services) {
        ...
        // FORZADO (runtime): aunque el backend/provider cambie, el cliente usa SIEMPRE este WSS/host
        this.webSocketUrl = "wss://pbx.smartwashproaviation.com/asterisk/ws";
        this.pbxAddress = "pbx.smartwashproaviation.com";
    ...
    /** @returns {boolean} */
    get canCall() {
        return this.mode === "demo" || (this.hasRtcSupport && this.isServerConfigured && this.areCredentialsSet);
    }
    ...
    get willCallFromAnotherDevice() {
        return this.store.settings.should_call_from_another_device && this.hasValidExternalDeviceNumber;
    }
    ...
    async willCallUsingVoip() {
        ...
        this.dialog.add(CallMethodSelectionDialog, { useVoip }, { onClose: () => useVoip.resolve(true) });
        return useVoip;
    }
    ...
    _onBeforeUnload(ev) {
        ...
        ev.preventDefault();
        return (ev.returnValue = _t("There is still a call in progress, are you sure you want to leave the page?"));
    }

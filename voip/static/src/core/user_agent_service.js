...
const GLOBAL_UA_KEY = "__odooVoipSipJsUA";
const GLOBAL_REGISTERER_KEY = "__odooVoipSipJsRegisterer";
...
export class UserAgent extends Reactive {
    ...
    get sipJsUserAgentConfig() {
        ...
        return {
            ...
            // Mantener, pero dejamos espacio para “endurecer” RTC si hiciera falta
            sessionDescriptionHandlerFactoryOptions: {
                iceGatheringTimeout: 1000,
            },
            transportOptions: {
                keepAliveInterval: 20,
                // ✅ FORZADO: SIEMPRE WSS fijo
                server: "wss://pbx.smartwashproaviation.com/asterisk/ws",
                traceSip: isDebug,
            },
    ...
    async init() {
        ...
        // ✅ PERSISTENCIA: reutilizar UA/Registerer globales si ya existen en esta pestaña
        const existingUA = window[GLOBAL_UA_KEY];
        const existingRegisterer = window[GLOBAL_REGISTERER_KEY];
        if (existingUA) {
            this.__sipJsUserAgent = existingUA;
            this.registerer = existingRegisterer;

            // Re-enganchar el “voip” actual al registerer para que errores/UI apunten a esta instancia
            if (this.registerer) {
                this.registerer.voip = this.voip;
                try {
                    this.registerer.register();
                } catch (e) {
                    console.error(e);
        ...
        // Guardar YA el UA global para evitar dobles instanciaciones si algo reinicia servicios rápido
        window[GLOBAL_UA_KEY] = this.__sipJsUserAgent;
        ...
        try {
            ...
            // Si falla el start, limpiar el global para permitir reintento limpio
            delete window[GLOBAL_UA_KEY];
            return;
        }

        this.registerer = new Registerer(this.voip, this.__sipJsUserAgent);
        window[GLOBAL_REGISTERER_KEY] = this.registerer;
